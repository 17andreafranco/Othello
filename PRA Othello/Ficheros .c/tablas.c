/**
 *  @file    	tablas.c
 *  @brief   	Procedimientos generales para la trabajar con tablas, cadenas y matrices.
...
{
    /* Variables locales */
	bool iguales,rang;           /* Variable booleana */
	int incc, incf,i,j;

	/* Inicializa las variables */
	i = *f;
	j = *c;
	/* Completa el código: búsqueda o recorrido? */
    iguales = false;

    tb_calcula_incrementos(orientacion, &incf,&incc);

    rang = tb_dentro_limites(i, j, nfilas, mcols);

	while ((rang==true)&&(iguales==false))
	{
	   i=i+incf;
	   j=j+incc;
...
{
	int fila,col,orientacion, i;

	/* 1. Recorrer el sub-cuadrado, y para cada celda:
	 - Si es válida, el carácter coincide y no es la casilla central, añadir a la tabla.
	 */
    i = 0;
    orientacion = 0;

    /* Añadir el centinla */
	for (fila =f-1;fila<=f+1;fila++)
	{
	    for(col=c-1;col<=c+1;col++)
	    {
	        if(tb_dentro_limites(fila, col, nfilas, mcols))
	        {
	            if((matriz[fila][col]==caracter)&&((fila!=f)||(col!=c)))
	            {
...
char lee_caracter_cadena (char cadena[], char texto[])
{
    ...
    scanf ("%c", &letra);
    /* Código */
    per = pertenece_cadena (letra, cadena);
    while (per != true)
    {
        scanf (" %c", &letra);
        per = pertenece_cadena (letra, cadena);
    ...
    /* Parámetros de entrada */		char caracter)
{
    ...
    *c=0;
    ...
    /* 2. Completa el algoritmo */
    while ((*f < nfilas)&&(trobat != true)){
        *c = 0;
        while((*c < mcols)&&(trobat != true)){
    ...
    /* Parámetros de salida */		int *incf, int *incc)
{
    ...
    fp_tb_calcula_incrementos(orientacion, incf, incc);
}
